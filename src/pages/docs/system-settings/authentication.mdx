...
<Callout type="info" emoji="️💡"> 
  Configure the LDAP TLS certificates

  Upload the following files to the default directory `/data/jumpserver/core/data/certs`:
    * `ldap_ca.pem` or `ldap_ha_ca.pem`
    * `ldap_cert.pem` or `ldap_ha_cert.pem`
    * `ldap_cert.key` or `ldap_ha_cert.key`
