...
4. In the **Server** field, enter the LDAP server URI, such as `ldap://example.com:389` or `ldaps://example.com:636`. If you need to configure a TLS certificate, upload the `ldap_ca.pem`, `ldap_cert.pem`, and `ldap_cert.key` files to the default directory `/data/jumpserver/core/data/certs`, then restart the service.
